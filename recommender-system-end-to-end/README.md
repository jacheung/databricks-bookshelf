...
### `00_data_preparation`
Generates a synthetic dataset of **15,000 historical cart transactions** across 2,000 users for a pizza chain. Performs basic data cleansing (removes sauces and utensils that should never be recommended) and saves three tables:

| Table | Purpose |
|---|---|
| `cleaned_mapped_dataset` | Full cleaned order history |
| `train_dataset` | 80 % training split (seed=42) — used by both downstream notebooks |
| `test_dataset` | 20 % held-out test split — shared across both notebooks for fair Hit@k comparison |
...
3. **Deployment** — two serving patterns:
   ...
   * **3b — Lakebase-backed PyFunc** (`MBALakebaseRecommenderModel`) — rules read from an online table at load time (for large rule sets or always-fresh rules)
...
3. **Deployment** — three steps:
   ...
   * **3b — Lakebase publish** — enables CDF, adds primary key, publishes to online store for point-lookup serving
...
|---|---|---|---|---|
| ALS (logged-in) | `ALSRecommenderModel` | Lakebase online table lookup | `mpid` | O(1) per user |
...
1. **Run `00_data_preparation`** — generates synthetic data, saves `train_dataset` and `test_dataset` to Unity Catalog
2. **Run `01_market_basket_analysis`** — trains FPGrowth on training data, evaluates on shared test set, logs PyFunc to Unity Catalog
3. **Run `02_collaborative_filter`** — trains ALS with HPO, evaluates on shared test set, publishes recommendations to Lakebase

All notebooks use `catalog = 'users'` and `schema = 'jon_cheung'` by default — update the Configuration cell (cell 3) in each notebook to point to your own schema.

## Configuration

Each notebook has a Configuration cell (cell 3) that defines all user-configurable names. No table names, model names, or store names are defined anywhere else.
...
| `online_store_name` | `pizza-chain-online-store` | Primary Lakebase store |
| `online_store_fallback` | `demo-online-store` | Fallback if quota is reached |
| `online_table_name` | `users.jon_cheung.association_rules_online` | Online table for rules |
...
| `online_store_name` | `pizza-chain-online-store` | Primary Lakebase store |
| `online_store_fallback` | `demo-online-store` | Fallback if quota is reached |
| `online_table_name` | `users.jon_cheung.als_recommendations_online` | Online table for recs |
...
| **Compute** | Classic cluster (all three notebooks) |
| **Packages** | `optuna`, `databricks-feature-engineering` (installed in-notebook) |
