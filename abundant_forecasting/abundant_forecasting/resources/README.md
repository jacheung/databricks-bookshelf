...
The databricks CLI bundles top level is defined by file `abundant_forecasting/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`abundant_forecasting/resources/model-workflow-resource.yml`)
 - batch inference workflow (`abundant_forecasting/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`abundant_forecasting/resources/monitoring-resource.yml`)
 - feature engineering workflow (`abundant_forecasting/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`abundant_forecasting/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `abundant_forecasting/databricks.yml` and `abundant_forecasting/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-abundant_forecasting-model     | /dev-abundant_forecasting-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-abundant_forecasting-model | /staging-abundant_forecasting-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-abundant_forecasting-model    | /prod-abundant_forecasting-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-abundant_forecasting-model    | /test-abundant_forecasting-experiment    |
...
2. Complete the `TODO` in `abundant_forecasting/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `abundant_forecasting/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `abundant_forecasting/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-abundant_forecasting-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-abundant_forecasting-batch-inference-job`
that runs the notebook under `abundant_forecasting/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-abundant_forecasting-batch-inference-job`.
...
`test-abundant_forecasting-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-abundant_forecasting-batch-inference-job`.
